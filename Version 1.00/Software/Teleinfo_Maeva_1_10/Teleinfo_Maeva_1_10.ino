...
const char version_logiciel[6] = "V1.10";
...
void setup() 
{
  ...
  pinMode(chipSelect, OUTPUT);     // Activation de la carte SD
  ...
  pinMode(ecritSD, OUTPUT);
  digitalWrite(ecritSD, LOW);     // Eteint par défaut
...
void loop()                     // Programme en boucle
{
  ...
  {     // Horloge RTC configurée
    DateTime now = RTC.now();     // lecture de l'horloge
    minute = now.minute();
    heure = now.hour();
    seconde = now.second();

    // Mise à jour variables date-heure à minuit.
...
void read_teleinfo()
{
  ...
  {
    if (ecritLog) ecritLog = false; // Gestion du message absence donnée.
    presence_teleinfo = 0;
...
{
  sprintf(mois_jour,"%02d/%02d/%02d,", annee, mois, jour);
}
